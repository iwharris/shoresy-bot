...
export interface IMatchers {
    getMatch(text: string, usernames: string[]): IMatcher;
}

export interface IMatcher {
    isMatch(text: string, usernames: string[]): boolean;
}

export class Matcher implements IMatcher {

	private terms: any;
	private chirps: string[];

	constructor(terms: any, chirps: string[]) {
		this.terms = terms;
		this.chirps = chirps.map(_.template);
	}
...
const chirps = [
    ...
    "Let's get some fucking gyozas!",
    "Your mom love buttplay like I like Häagen-Dazs, let's get some fuckin' ice cream!",
    'Fuck your entire fucking life ya piece of shit!',
    'Make yourselves useful, grab me a bag of dill picklers!',
    'Fuck you all, your lives are so sad, I get a charity tax break just for hanging out with ya! Nice sweep, no swee, give yer balls a tug!',
];
